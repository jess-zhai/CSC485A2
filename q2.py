...
def gather_sense_vectors(corpus: T.List[T.List[WSDToken]],
    ...
    * That yields a single vector associated to each synset; return this as
    ...
    """
    synset_vectors = defaultdict(list)
    ...
    for batch_n in trange(0, len(corpus), batch_size, desc='gathering',
        ...
        ### START CODE HERE
        bert_output, offset_mappings = run_bert([[token.wordform for token in sentence] for sentence in batch])
        offset_mappings = offset_mappings.tolist()
        for sent_index, sentence in enumerate(batch):
            offset_mapping = offset_mappings[sent_index]
            bert_sentence_output = bert_output[sent_index]        
            align = []
            ...
            for token, (start, end) in zip(sentence, align):
                if token.synsets:
                    token_vectors = bert_sentence_output[start:end]
                    token_vector = token_vectors.mean(dim=0)
                    for synset in token.synsets:
                        synset_vectors[synset].append(token_vector)

    synset_mean_vectors = {synset: torch.stack(vectors).mean(dim=0) for synset, vectors in synset_vectors.items()}
...
def cosine_similarity(tensor1: torch.Tensor, tensor2: torch.Tensor) -> torch.Tensor:
    ...
    norm_tensor1 = tensor1.norm(dim=1, keepdim=True)
    norm_tensor2 = tensor2.norm(dim=1, keepdim=True)
    return dot_product / (norm_tensor1 * norm_tensor2.T)
...
def bert_1nn(batch: T.List[T.List[WSDToken]],
    ...
    **IMPORTANT**: When computing the cosine similarities and finding the sense
    ...
    """
    ### START CODE HERE
    bert_outputs, offset_mappings = run_bert([[token.wordform for token in sentence] for sentence in batch])
    predictions = []
    for sentence_id, (sentence, target_indices) in enumerate(zip(batch, indices)):
        sentence_pred = []
        offset_mapping = offset_mappings[sentence_id].tolist()
        bert_sentence_output = bert_outputs[sentence_id]
        align = []
        for i, (start, end) in enumerate(offset_mapping):
            if [start, end] == [0, 0]:
                continue
        ...
        for token_index in target_indices:
            ...
            synsets = wn.synsets(token.lemma) # NOT THE GIVEN ONE!!!!!!!!!! AHHHHHHH!!!!!!!!! I WAS STUCK AT 93% FOR 6 HOURS!!!!!
            if not synsets:
                sentence_pred.append(mfs(sentence, token_index))
                continue

            valid_synsets = [synset.name() for synset in synsets if synset.name() in sense_vectors] # need this
            if not valid_synsets:
                sentence_pred.append(mfs(sentence, token_index))
                continue

            sense_matrix = torch.stack([sense_vectors[synset_name] for synset_name in valid_synsets])
            bert_token_range = align[token_index]
            bert_token_output = bert_sentence_output[bert_token_range[0]:bert_token_range[1]].mean(dim=0)

            similarities = cosine_similarity(bert_token_output.unsqueeze(0), sense_matrix)
            best_id = similarities.argmax()
            best_synset_id = valid_synsets[best_id] #changed
            sentence_pred.append(wn.synset(best_synset_id))
