...
def overlap(signature: List[str], context: List[str]) -> int:
    ...
    overlap_count = 0
    for word in signature_count:
        overlap_count += min(signature_count[word], context_count.get(word, 0))
...
def lesk(sent: Sequence[WSDToken], word_index: int) -> Synset:
    ...
    **IMPORTANT**: when looking up the word in WordNet, make sure you use the
    ...
    best_score = 0
    target_word = sent[word_index]
    context = [token.wordform for token in sent]
    context = stop_tokenize(' '.join(context))
    for synset in wn.synsets(target_word.lemma):
        signature = stop_tokenize(synset.definition())
...
def get_related_words(synset: Synset) -> List[str]:
    related_words = []
    related_words.extend(stop_tokenize(synset.definition()))
    for example in synset.examples():
        related_words.extend(stop_tokenize(example))
    
    for rel_type in [synset.hyponyms, 
        ...
        for related_synset in rel_type():
            related_words.extend(stop_tokenize(related_synset.definition()))
...
def lesk_ext(sent: Sequence[WSDToken], word_index: int) -> Synset:
    ...
    **IMPORTANT**: when looking up the word in WordNet, make sure you use the
    ...
    for synset in wn.synsets(target_word.lemma):
        signature = get_related_words(synset)
...
def vectorize(bag_of_words: List[str], vocabulary: List[str]) -> List[int]:
    vector = [0] * len(vocabulary)
    word_counts = Counter(bag_of_words)
    for index, word in enumerate(vocabulary):
        vector[index] = word_counts[word]
    return vector
...
def lesk_cos(sent: Sequence[WSDToken], word_index: int) -> Synset:
    ...
    **IMPORTANT**: when looking up the word in WordNet, make sure you use the
    ...
    for synset in wn.synsets(target_word.lemma):
        signature = get_related_words(synset)
        vocabulary = set(context).union(set(signature))
        context_vector = vectorize(context, vocabulary)
        signature_vector = vectorize(signature, vocabulary)

        score = cosine_similarity(signature_vector, context_vector)
        if score > best_score:
            best_sense = synset
            best_score = score
...
def lesk_cos_onesided(sent: Sequence[WSDToken], word_index: int) -> Synset:
    ...
    **IMPORTANT**: when looking up the word in WordNet, make sure you use the
    ...
    for synset in wn.synsets(target_word.lemma):
        signature = get_related_words(synset)
        signature = [token for token in signature if token in context]
        vocabulary = set(context).union(set(signature))
        context_vector = vectorize(context, vocabulary)
        signature_vector = vectorize(signature, vocabulary)
...
def get_word_vector(word: str, vocab: Mapping[str, int], word2vec: np.ndarray) -> np.ndarray:
    word = word.replace(" ", "_")
    if word in vocab:
        ...
        words = word.split("_")
        if len(words) > 1:
            vectors = [get_word_vector(w, vocab, word2vec) for w in words]
...
def lesk_w2v(sent: Sequence[WSDToken], word_index: int,
    ...
    * If the version with underscores doesn't yield anything, split the
    ...
    context = [token.wordform for token in sent]
    context = stop_tokenize(' '.join(context))
    context_vectors = np.array([get_word_vector(word, vocab, word2vec) for word in context])
    context_vector = np.mean(context_vectors, axis=0)
    for synset in wn.synsets(target_word.lemma):
        signature = set(get_related_words(synset))
        signature_vectors = np.array([get_word_vector(word, vocab, word2vec) for word in signature])
...
if __name__ == '__main__':
    ...
    #for wsd_func in [lesk_cos, lesk_cos_onesided]:
